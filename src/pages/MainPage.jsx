...
function MainPage({ setScore, getI, dark, cilckDark }) {
  // vaqtni nazorat qilish un
  const [start, setStart] = useState(false);
  //  page ni o'zgartirishimiz un kk
  const [changeLink, setChangeLink] = useState(false);
  // savolni o'zgartirishimiz un kk
  const [changeQuiz, setChangeQuiz] = useState(0);
  // sel element tanlanganligini bilimish un kk
  const [sel, setSel] = useState(false);
  const [changeCount, setChangeCount] = useState(1);
  //sum btn dagi textni nazorat qilish uchun kk
  const [sum, setSum] = useState(false);
  const [count, setCount] = useState(false);
  let savolniUzgartir = false;
  ...
  return (
    <div className={`h-full ${dark ? "app2 dark" : "app"}`}>
      ...
      <div className="container main-page">
        <section className="m-sec-left">
          <p className="m-p">Question {changeQuiz + 1} of 5</p>

          <h1 className="m-h1">{getI[changeQuiz]?.question}</h1>
          <div className="timer">
            <div className={`time ${start ? "start-timer" : ""}`}></div>
          </div>
        </section>
        <section className="m-sec-right">
          {getI[changeQuiz]?.answers.map((answer, i) => {
            return (
              <div onClick={() => celacted(i)} key={i} className="options">
                <p className="a">{i + 1}</p>
                <p>{answer}</p>
              </div>
            );
